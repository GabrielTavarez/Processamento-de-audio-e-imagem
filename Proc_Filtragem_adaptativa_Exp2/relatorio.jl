...
md" # Exp 2 - Curvas de Aprendizado
...
Como estamos usando $$\Phi_i = x[n-i]$$ e a matriz de autocorrelação $R_\Phi$ é definida por
...
$$
	...
	E\{x[n]²\} = 
	E\{cos²(ω_0 n + \Theta)\} = 
	\int cos²(ω_0 n + \Theta)\frac{1}{2\pi}d\Theta = 
	\frac{\pi}{2\pi} = 
	\frac{1}{2}
.$$ 
...
$$
	E\{\Phi_0 \Phi_1 \}  = 
	E\{x[n] \cdot x[n-1]\} = 
	\int cos(ω_0 n + \Theta)cos(ω_0 [n-1] + \Theta)\frac{1}{2\pi}d\Theta =  
.$$

$$
	= 
	\frac{\pi cos(ω_0 )}{2\pi} = 
	\frac{cos(ω_0)}{2}
.$$ 
...
\left[
	\begin{array}{cc} 
		\frac{1}{2} & \frac{cos(ω_0)}{2}\\
		\frac{cos(ω_0)}{2} & \frac{1}{2}
	\end{array}
\right] =
\left[
	\begin{array}{cc} 
...
begin
	N = 2500
	n = 1:N
	Θ = rand(1)*2π # fase de x 
	Ψ = rand(1)*2π # fase de v

	ω0 = 0.5π # freq de x
	ω1 = 0.01π # freq de v
	A = 0.001 # amplitude de v
	...
	M = 2 #coeficientes do filtro estimado
...
begin
	p1 = plot(erro2, label = "Erro (e[n])")
	p2 = plot(log.(abs.(Δw0_2)), color = :green, width = 2, label = "Δw0")
	plot!(log.(abs.(Δw1_2)), color= :red, width = 2, label = "Δw1")
...
begin
	plot(Δw0, label = "Δw₀")
	plot!(Δw1, label = "Δw₁")
	plot!(title = "Erro dos coeficientes do filtro estimado", legend = (0.87,0.85))
end

# ╔═╡ 5fa5f019-3174-4c9d-a1ba-2872ad95b379
md" A taxa de convergência é definida por 

$taxa = log(1-\mu \lambda_{max})$

portanto temos a taxa teórica de 

**taxa = $(round(log(1-0.05*0.5), digits = 5))**

Olhando no gráfico de experimental de Δωᵢ podemos calcular a taxa experimental com 

$$
taxa = \frac{d log(\Delta w)}{d n}$$

portanto temos a taxa experimental de 

**taxa = $(round((log(Δw0[5])-log(Δw0[1]))/4, digits = 5))**

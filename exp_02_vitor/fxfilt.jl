...
Filters signal `x` by an IIR filter with numerator coefficients in vector `b/a[1]`, and denominator coefficients in vector `a/a[1]`, using the Direct I form
and initial conditions in vector `zi`.
"""
function fxfilt(b::Real,a::Vector,x::Vector, zi::Vector)
    ...
    xext = [zeros(N-1);x]
    for n = 1:Nx
        y[n+N-1] = -anorm⋅y[n+N-2:-1:n+N-N] + bnorm*xext[n]
    end
    return y[N:end]
end
...
and initial conditions in vector `zi`.
"""
function fxfilt(b::Vector,a::Vector,x::Vector, zi::Vector)
...
Filters quantized signal `x` by an IIR filter with numerator coefficients in vector `b/a[1]`, and denominator coefficients in vector `a/a[1]`, using the Direct I form.
...
Filters quantized signal `x` by an IIR filter with quantized numerator coefficients in vector `b/a[1]`, and quantized denominator coefficients in vector `a/a[1]`, using the Direct I form.
